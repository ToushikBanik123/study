...
bool isPalindrome(Node *head, Node *&mid, int pos, int len, int count = 0) {
  ...
  if (isPalindrome(head->next, mid, pos, len, count + 1)) {
...
int main() {
  ...
  for (int i = 0; i < tc; i++) {
    // 		int target;
    pair<int, pair<Node *, Node *>> input1 = takeInput();
    pair<int, pair<Node *, Node *>> input2 = takeInput();
    Node *head1 = input1.second.first;
    Node *head2 = input2.second.first;
    ...
    // 		printLL(head);
    // 		printLLReverse(head);
    // 		cout << isPalindrome(head, head, (len - 1)/2, len);
