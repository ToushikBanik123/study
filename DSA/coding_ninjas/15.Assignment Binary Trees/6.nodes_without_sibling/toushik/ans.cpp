...
template <typename T>
void printSibling(TreeNode<T> * rootNode){
    if(rootNode == NULL) return;

    if(rootNode->left && rootNode->right == NULL)
    cout << rootNode->left->data << " ";
    if(rootNode->right && rootNode->left == NULL)
    cout << rootNode->right->data <<" ";

    if(rootNode->left) printSibling(rootNode->left);
    if(rootNode->right) printSibling(rootNode->right);
}

int main(){
    TreeNode<int>* rootNode = takeInput<int>();
    printSibling<int>(rootNode);
    return 0;
}
