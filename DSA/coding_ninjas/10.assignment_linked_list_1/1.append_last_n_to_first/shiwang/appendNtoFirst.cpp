...
Node *appendNToFirst(Node *head, Node *temp, Node *tail, int pos,
                     int count = 0) {
  if (pos <= 0)
    return head;

  if (temp != NULL && count < pos - 1) {
    return appendNToFirst(head, temp->next, tail, pos, count + 1);
  }
...
int main() {
  ...
  for (int i = 0; i < tc; i++) {
    ...
    head = appendNToFirst(head, head, tail, len - target);
    printLL(head);
  }
}