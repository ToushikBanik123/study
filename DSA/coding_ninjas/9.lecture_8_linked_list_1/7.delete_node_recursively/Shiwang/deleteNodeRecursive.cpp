...
Node *deleteNodeRecursive(Node *head, int pos, int count = 0) {
  ...
  if (head != NULL && count < pos - 1) {
    deleteNodeRecursive(head->next, pos, count + 1);
    return head;
  }
...
int main() {
  ...
  for (int i = 0; i < tc; i++) {
    ...
    head = deleteNodeRecursive(head, pos);
    printLL(head);
  }
  ...
  // 	head = insertNodeRecursive(head, pos, data);
